...
class MysqlServer:
    ...
    def __init__(self, host, port, db, user, passwd, charset='utf8'):
        ...
        logging.debug('Connect MySQL: host={host}, port={port}, db={db}, user={user}, passwd={passwd}'
                      .format(host=host, port=port, db=db, user=user, passwd=str(passwd)))
        try:
            self.conn = pymysql.connect(host=host, port=port, db=db, user=user, passwd=str(passwd), charset=charset)
